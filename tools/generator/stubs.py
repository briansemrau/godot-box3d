"""Generate C++ binding stubs for Box3D API functions."""
...
def generate_stub(func: Function, type_map: dict, blobs: set, data: dict) -> str:
    """Generate a C++ method stub for a Box3D function."""
    from array import _emit_return_lines
...
def collect_generatable(root: str, type_map: dict) -> dict:
    ...
    Uses :func:`classification.classify_function` as the single source of
    truth. Domain is purely a grouping key for output files — no function is
    ever dropped for landing in the "other" bucket.
    """
    from classification import GENERATE, classify_function, prepare_type_map
...
def write_output(root: str, domains: dict, type_map: dict):
    ...
    header_path.write_text(
        "#pragma once\n\n"
        "// Auto-generated by tools/generator/stubs.py — do not edit.\n"
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Generate Box3D API binding stubs")
    parser.add_argument("root", nargs="?", default=".", help="Project root directory")
    args = parser.parse_args()

    type_map = load_type_map(args.root)
    domains = collect_generatable(args.root, type_map)

    total = sum(len(f) for f in domains.values())
    print(f"Generating stubs for {total} functions across {len(domains)} domains:")
    for domain, funcs in sorted(domains.items()):
